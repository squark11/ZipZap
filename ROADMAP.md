...
- 🔧 **P‑Role2 — Rejestracja per‑kanał.** ✅ (2026-09-15): backend `POST /api/register/store` (sklep + właściciel `StoreEmployee`, zwraca auth), `POST /api/register/driver` (`Driver` **pending/nieaktywny**), `GET /api/admin/drivers/pending` + `POST /api/admin/drivers/{id}/approve` (Admin: przypisz sklep + aktywuj) — captcha-gated. Panel: ekran logowania przełącza **Logowanie / „Załóż sklep" / „Zostań dostawcą"** (rejestracja sklepu loguje i wchodzi w onboarding; dostawca → komunikat „oczekuje na weryfikację"). Testy `RegistrationTests` (3, live-smoke OK). ⬜ **wdrożenie panelu** (konfiguracja API base URL) + **UI zatwierdzania dostawców** w panelu (Zespół/Admin) + widget captcha w panelu. Kanały:
